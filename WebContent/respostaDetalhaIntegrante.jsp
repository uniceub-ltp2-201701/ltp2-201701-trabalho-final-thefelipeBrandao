	...
	<body>
		<div id="interface">	
			...
			<fieldset id="informacoesDoIntegrante">
				...
				<strong>Gênero: </strong><a id="link" href="listaIntegrantesDeUmGenero?genero=<%= integrante.getGenero()%>&generoNome=<%= integrante.getGeneroNome()%>"><%= integrante.getGeneroNome()%></a><br>
				...
				<% for(Profissao p : profissoes) { %>
						/<a id="link" href="listaFilmesDeUmaProfissaoDeUmIntegrante?idProfissao=<%=p.getIdProfissao()%>&idIntegrante=<%=integrante.getIdIntegrante()%>"><%=p.getNomeProfissao()%></a>			    	
				<%  }  %><br>
				<strong>Filme(s) feito(s): </strong>
				<% for(Filme f : filmes) { %>
						/<a id="link" href="detalhaFilme?idFilme=<%= f.getIdFilme() %>"><%= f.getNomeFilme() %></a>			    	
				<%  }  %>
			</fieldset><br>
